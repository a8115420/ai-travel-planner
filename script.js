...
let currentItineraryId = null; // 用來追蹤當前載入的行程 ID
...
function initializePage() {
    ...
    // 檢查 URL 中是否有 Google Sync 成功的參數
    const urlParams = new URLSearchParams(window.location.search);
    if (urlParams.get('sync_success') === 'true') {
        showToast('行程已成功同步至您的 Google 日曆！');
        // 清理 URL，避免重新整理時再次顯示
...
async function handleGoogleSync() {
    if (!currentItineraryId) {
        return showToast('請先從「我的行程」列表中載入一個行程來進行同步。', 'error');
    }
    const startDate = document.getElementById('start-date').value;
    const endDate = document.getElementById('end-date').value;
    const token = localStorage.getItem('token');
    if (!startDate || !endDate) return showToast('請選擇行程的開始與結束日期！', 'error');
    if (new Date(startDate) >= new Date(endDate)) return showToast('結束日期必須在開始日期之後！', 'error');
    try {
        const response = await fetch(`${API_BASE_URL}/api/google/prepare-sync`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
            body: JSON.stringify({ itineraryId: currentItineraryId, startDate, endDate })
        });
...
async function handleSendMessage() { const chatInput = document.querySelector('.chat-input-area input'); const chatWindow = document.querySelector('.chat-window'); const userMessage = chatInput.value; if (!userMessage.trim()) return; appendMessage(userMessage, 'user-message', chatWindow); conversationHistory.push({ role: 'user', content: userMessage }); chatInput.value = ''; const loadingBubble = document.createElement('div'); loadingBubble.className = 'loading-bubble'; loadingBubble.innerHTML = '<div class="dot"></div><div class="dot"></div><div class="dot"></div>'; chatWindow.appendChild(loadingBubble); chatWindow.scrollTop = chatWindow.scrollHeight; try { const response = await fetch(`${API_BASE_URL}/api/chat`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ message: userMessage }), }); loadingBubble.remove(); if (!response.ok) { const errorData = await response.json().catch(() => ({ error: '無法解析錯誤訊息' })); throw new Error(errorData.error || 'Network response was not ok'); } const data = await response.json(); appendMessage(data.reply, 'ai-message', chatWindow); conversationHistory.push({ role: 'ai', content: data.reply }); } catch (error) { console.error('Fetch error:', error); loadingBubble.remove(); appendMessage(`與 AI 連線時發生錯誤... (${error.message})`, 'ai-message', chatWindow); } }
